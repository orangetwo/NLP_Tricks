...
def MaskForDecoder(x, pad_idx, mode='decoder'):
	...
	# This function implementation rely on your Transformer structure
	...
	assert mode in ('decoder', 'encoder'), f"check the mode parameter!!!"
...
def beam_search_generate(encoder,
                         decoder,
                         encoder_input,
                         vocab_size,
                         batch_size=3,
                         ...
                         min_length=2,
                         num_beams=2,
                         encoder_bos_token_id=101,
                         encoder_pad_token_id=0,
                         encoder_eos_token_id=102,
                         bos_token_id=101,
                         pad_token_id=0,
                         eos_token_id=102,
	...
	encoder_output = encoder(encoder_input, mask4encoder)
	...
	while cur_len < max_length:
		...
		logit = decoder(encoder_output, input_ids, mask4decoder, mask4encoder)
		...
		for batch_idx in range(batch_size):
			...
			"""
                       ...
                       1. 已经记录过该样本结束
                       2. 在已经生成beam size个含有eos token的序列前提下, 如果当前生成的序列的得分在任何情况下(长度: current length -> max length )
                           都差于已经生成的beams size个序列 那么 将 该样本结束搜索

                       这里 当前生成的序列的得分在任何情况下 的含义 值得 假设 我们有一个序列 score("我想回家")/current_length 肯定小于 score("我想回家")/max_length
                       (假设了 max_length 大于 current_length),注意score是负数
            """
			done[batch_idx] = done[batch_idx] or generated_hyps[batch_idx].is_done(
				next_scores[batch_idx].max().item(), cur_len=None
			)
	...
	return generated_hyps, input_ids, beam_scores
